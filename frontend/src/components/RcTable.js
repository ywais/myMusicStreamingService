  ...
  if(songs.length && songs[0][0].Title !== null) {
    ...
    const data = songs.map(song => {
      let songInfo = Object.keys(song[0]).map(key => {
        if(key === 'Title') {
          return {[key]: <Link to={`/song/${song[1].songId}?${type}=${song[1].myId}`}>{song[0][key]}</Link>};
        } else if(key === 'Artist') {
          return {[key]: <Link to={`/artist/${song[1].artistId}`}>{song[0][key]}</Link>};
        } else if(key === 'Album') {
          return {[key]: <Link to={`/album/${song[1].albumId}`}>{song[0][key]}</Link>};
        } else {
